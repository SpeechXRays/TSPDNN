...
function M.parse(arg)
   ...
   cmd:option('-data',
              paths.concat(os.getenv('HOME'), 'DEEPLEARNING', 'openface', 'data',
                           'FRGC/',
                           'aligned'),
   ...
   cmd:option('-peoplePerBatch',20, 'Number of people to sample in each mini-batch.')
   cmd:option('-imagesPerPerson', 18, 'Number of images to sample per person in each mini-batch.')
   cmd:option('-testing', true, 'Test with the LFW.')
   cmd:option('-testBatchSize', 70, 'Batch size for testing.')
